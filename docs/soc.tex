...
vygenerované pred tým, ako sa simulácia začne. Po spustení simulácie nie sú tieto
...
\begin{figure}[h]
	...
	\caption{Na obráyku je znázornená pohyblivá štruktúra (červená), ktorá
...
správania a vývinu simulovaných rastlín vďaka schostiam:
...
Veríme, že metódy využité v Botanice sú využiteľné ako na vytváranie dynamického
...
Čo sa týka algoritmu na jeho generovanie (obr. \ref{obr:perlinov sum}), v určitých miestach sa vytvoria
...
oktáva, avšak vplýva menej na výslednú hodnotu. Vyjadrenie hodnôt bodov
...
Aby nebola simulácia len v podobe čísiel v súbore, sú tieto voxely aj
vykresľované na obrazovku. Renderovanie je prevedené na grafickú kartu,
...
Ako už bolo spomínané v časti \ref{subsec:svet}, svet je zložený z malých
kociek (voxelov) v sieti 32x32x32. Voxely môžu byť tovrené rôznymi materiálmi,
čo je symbolizované číselnou hodnotou pre každú jednu bunku v sieti. Tieto
...
polohovými hodnotami, ktoré následne každý snímok vykreslí na obrazovku.
...
(povedľa seba), čo im umožňuje sa "prehrýzť" obrovským množstvom dát za okamih,
avšak je to niečo, na čo treba myslieť pri programovaní výpočtových programov,
compute shaderov.

Shader je spustený niekoľko krát, raz na každý voxel. Jeho výstupom je
neprerušená pamäť vertexov ktoré sú následne zaslané na renderovanie.
...
Dáta sú shaderu poslané v jednom veľkom kuse pamäti, čiže shader potrebuje
vedieť, kde sa jemu priradený voxel v tejto pamäti nachádza.

Každá invokácia shaderu (každý spustený program) dostane poradové číslo, ktoré
je použité ako index, poradník, voxelu v pamäti z ktorej je vytiahnutá nielen
hodnota jemu priradenému voxelu, ale aj každého susedného voxelu. Tie sú
následne využité na kontrolu stien
